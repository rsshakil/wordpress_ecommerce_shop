			...
			public function query() {
				...
				if ( $portfolio_query->have_posts() ) {
					while ( $portfolio_query->have_posts() ) {
						...
						$element_orientation_class = $fusion_library->get_images_obj()->get_element_orientation_class( get_post_thumbnail_id(), $post_thumbnail_attachment, self::$args['portfolio_masonry_grid_ratio'], self::$args['portfolio_masonry_width_double'] );
					...
					if ( has_post_thumbnail() || $fusion_settings->get( 'featured_image_placeholder' ) || fusion_get_page_option( 'video', $this->post_id ) ) {
						...
						if ( 'carousel' === $layout ) {
							...
							if ( ! has_post_thumbnail() && fusion_get_page_option( 'video', $this->post_id ) ) {
								...
								// Get the post image.
								if ( 'full' === $this->image_size && class_exists( 'Avada' ) && method_exists( Avada(), 'get_images_obj' ) ) {
								...
								if ( class_exists( 'Avada' ) && method_exists( Avada(), 'get_images_obj' ) ) {
									Avada()->get_images_obj()->set_grid_image_meta( [] );
								}
							}
							...
							if ( ! has_post_thumbnail() && fusion_get_page_option( 'video', $this->post_id ) ) {
								...
								if ( 'masonry' === $layout ) {
									...
									// Get the correct image orientation class.
									if ( class_exists( 'Avada' ) && method_exists( Avada(), 'get_images_obj' ) ) {
										$element_orientation_class = Avada()->get_images_obj()->get_element_orientation_class( get_post_thumbnail_id(), $post_thumbnail_attachment, $defaults['portfolio_masonry_grid_ratio'], $defaults['portfolio_masonry_width_double'] );
